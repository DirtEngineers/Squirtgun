...
public class FakeItemRenderer_OLD {
    ...
    private static MultiBufferSource getWrappedBuffer(MultiBufferSource pBufferSource, float pAlpha) {
        return (pRenderType) -> new WrappedVertexConsumer_OLD(pBufferSource.getBuffer(RenderType.entityTranslucent(InventoryMenu.BLOCK_ATLAS)), 1.0F, 1.0F, 1.0F, pAlpha);
    }

    private static BakedModel getBakedModel(ItemStack pItemStack) {
        if (pItemStack.getItem() instanceof ChemicalItem || pItemStack.getItem() instanceof ElementItem) {
            ModelResourceLocation chemicalModel = getChemicalModel(pItemStack);
            if (chemicalModel != null) {
                return ITEM_RENDERER.getItemModelShaper().getModelManager().getModel(chemicalModel);
            }
        }
